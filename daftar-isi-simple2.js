         ...
         // get the post labels from the entry
            ...
            if ("category" in entry) {
               for (var k = 0; k < entry.category.length; k++) {
                  pll += '<a href="javascript:filterPosts(\'' + entry.category[k].term + '\');" title="Clic para seleccionar todas las publicaciones con etiqueta \'' + entry.category[k].term + '\'">' + entry.category[k].term + '</a>,  ';
               }
...
// this function creates a three-column table and adds it to the screen
   ...
   var tocTool1 = 'Clic para ordenar por título';
   var tocHead2 = 'FECHA DEL POST';
   var tocTool2 = 'Clic para ordenar por fecha';
   var tocHead3 = 'ETIQUETAS';
   var tocTool3 = '';
   if (sortBy == "titleasc") { 
      tocTool1 += ' (descendente)';
      tocTool2 += ' (reciente primero)';
   }
   if (sortBy == "titledesc") { 
      tocTool1 += ' (ascendente)';
      tocTool2 += ' (reciente primero)';
   ...
   if (sortBy == "datenewest") { 
      tocTool1 += ' (ascendente)';
      tocTool2 += ' (último primero)';
   }
   if (postFilter != '') {
      tocTool3 = 'Clic para mostrar todas las publicaciones';
   }
   tocTable += '<table>';
   tocTable += '<tr>';
   ...
   if (numDisplayed == postTitle.length) {
      var tocNote = '<span class="toc-note">Mostrando los ' + postTitle.length + ' posts<br/></span>'; }
   else {
...
function showToc() {
  ...
  else { alert("Solo espera ... TOC está cargando"); }
}
